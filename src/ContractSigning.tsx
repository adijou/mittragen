...
type SigningData = {
  spaceTarget: { tenantId: string; sponsorId: string };
  contract: {
  ...
  return <div className="contract-signing-page" style={style}>
    ...
    <main>
      ...
      {data && <>
        ...
        {confirmed ? <section className="contract-signing-card contract-signing-success"><span aria-hidden="true">✓</span><div><p className="eyebrow">Bestätigung abgeschlossen</p><h2>Vielen Dank für Ihre Unterstützung.</h2><p>Ihr Vertrag ist bestätigt. In Ihrem persönlichen Space finden Sie Ihre Dokumente, können Ihre Adresse ändern und Ihr Logo hinterlegen.</p><p>Erstellen Sie Ihren Zugang mit <strong>{data.signer.email}</strong> und bestätigen Sie Ihre E-Mail-Adresse. Ihr Sponsoring wird Ihnen automatisch zugeordnet.</p><div className="contract-signing-space-actions"><a className="access-primary" href="/login" onClick={() => prepareSponsorAccess({ email: data.signer.email, name: data.signer.name, target: data.spaceTarget, mode: "signup" })}>Persönlichen Space einrichten</a><a className="access-secondary" href="/login" onClick={() => prepareSponsorAccess({ email: data.signer.email, name: data.signer.name, target: data.spaceTarget, mode: "login" })}>Mit bestehendem Konto anmelden</a></div><small>Ihr Zugang bleibt auch nach Ablauf dieses Vertragslinks über mittragen.ch/sponsor erreichbar.</small>{error && <p className="form-error" role="alert">{error}</p>}</div></section> : <section className="contract-signing-card contract-signing-confirm">
