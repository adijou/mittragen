...
function PortalBrand() {
  return <div className="product-brand" aria-label="Mittragen">
    <svg viewBox="0 0 42 42" aria-hidden="true">
      <circle cx="21" cy="5.5" r="4.5" className="product-brand__blue"/><circle cx="34.5" cy="13" r="4.5" className="product-brand__gold"/>
      <circle cx="34.5" cy="29" r="4.5" className="product-brand__blue"/><circle cx="21" cy="36.5" r="4.5" className="product-brand__gold"/>
      <circle cx="7.5" cy="29" r="4.5" className="product-brand__blue"/><circle cx="7.5" cy="13" r="4.5" className="product-brand__gold"/>
      <circle cx="21" cy="21" r="6" className="product-brand__center"/>
    </svg><span>mittragen</span>
  </div>;
}
...
export function SponsorPortal({ onHome, onLogin }: { onHome: () => void; onLogin: () => void }) {
  ...
  const confirmContract = async (contract: SponsorContract) => {
    ...
    try {
      ...
      setError(code === "contract_acknowledgement_required" ? "Bitte bestätigen Sie, dass Sie den Vertrag geprüft haben und zur Bestätigung berechtigt sind." : code === "contract_already_confirmed" ? "Dieser Vertrag wurde bereits bestätigt." : "Der Vertrag konnte nicht bestätigt werden.");
    } finally { setBusy(""); }
  };
  ...
  return <div className="sponsor-portal">
    <header><button onClick={onHome} className="access-brand-button"><PortalBrand/></button><div><span>{user.email}</span><button className="access-link" onClick={() => void logout().then(() => setUser(null))}>Abmelden</button></div></header>
    <main>
      ...
      {!space ? <section className="sponsor-portal__welcome"><p className="eyebrow">Noch kein Zugang</p><h1>Keine offene Einladung gefunden.</h1><p>Prüfen Sie, ob Sie mit derselben E-Mail-Adresse angemeldet sind, an die die Einladung gesendet wurde. Abgelaufene Zugänge kann Ihre Organisation neu versenden.</p></section> : <>
        ...
        {space.contracts.length > 0 && <section className="sponsor-contracts">
          ...
          <div className="sponsor-contracts__list">{space.contracts.map((contract) => <article className={`sponsor-contract sponsor-contract--${contract.status}`} key={contract.id}>
            ...
            {contract.status === "released" ? <form className="sponsor-contract__confirm" onSubmit={(event) => { event.preventDefault(); void confirmContract(contract); }}><div><strong>Vertrag elektronisch bestätigen</strong><p>Bitte lesen Sie zuerst das PDF und bestätigen Sie nur, wenn Sie für den Sponsor zeichnungs- oder bestätigungsberechtigt sind.</p></div><label><span>Name der berechtigten Person</span><input required value={signingAuthorityName} onChange={(event) => setSigningAuthorityName(event.target.value)} autoComplete="name"/></label><label><span>Funktion beim Sponsor</span><input required value={signingAuthorityRole} onChange={(event) => setSigningAuthorityRole(event.target.value)} placeholder="z. B. Geschäftsführung"/></label><label className="sponsor-ack"><input type="checkbox" checked={contractAcknowledged} onChange={(event) => setContractAcknowledged(event.target.checked)}/><span>Ich habe den vollständigen Vertrag geprüft, bin zur Bestätigung berechtigt und stimme dem unveränderlichen Vertragsstand zu.</span></label><button className="access-primary" disabled={!contractAcknowledged || !signingAuthorityName.trim() || !signingAuthorityRole.trim() || busy !== ""}>{busy === `confirm-${contract.id}` ? "Wird bestätigt …" : "Vertrag verbindlich bestätigen"}</button><small className="sponsor-contract__legal-note">Dies ist eine protokollierte Klickbestätigung. Für Geschäfte mit gesetzlichem Schriftformerfordernis ist später eine fortgeschrittene oder qualifizierte Signatur vorgesehen.</small></form> : <div className="sponsor-contract__proof"><strong>Bestätigung protokolliert</strong><p>Das PDF enthält den freigegebenen Stand und den Bestätigungsnachweis.</p><code>{contract.snapshot_hash.slice(0, 16)}…</code></div>}
          </article>)}</div>
        </section>}
      </>}
    </main>
