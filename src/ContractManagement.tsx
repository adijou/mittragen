...
export function ContractManagement({ tenantId, canWrite, canManage }: { tenantId: string; canWrite: boolean; canManage: boolean }) {
  ...
  const saveSettings = async (event: React.FormEvent) => {
    event.preventDefault(); setBusy("settings"); setError(""); setMessage("");
    try {
      const result = await api<{ settings: Settings }>(`/api/contracts/${tenantId}/settings`, {
        method: "PATCH",
        body: JSON.stringify({ ...settings, noticeMonths: settings.renewalMode === "annual_auto" ? settings.noticeMonths : null }),
      });
      setSettings(result.settings); setShowSettings(false); setMessage("Vertragsangaben wurden gespeichert.");
    } catch { setError("Die Vertragsangaben konnten nicht gespeichert werden."); }
    finally { setBusy(""); }
  };
  ...
  return <section className="contract-management">
    <header><div><p className="eyebrow">Vertragscenter</p><h1>Verträge nachvollziehbar abschliessen</h1><p>Aus dem bestätigten Paket entsteht ein gebrandeter, unveränderlicher Vertragsstand mit sauberem Nachweis.</p></div>{canManage && <button className="access-secondary" onClick={() => setShowSettings((current) => !current)}>{showSettings ? "Angaben schliessen" : "Vertragsorganisation"}</button>}</header>
    {error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-success" role="status">{message}</p>}
    {showSettings && canManage && <form className="contract-settings" onSubmit={saveSettings}><div><p className="eyebrow">Absender und Rechtsgrundlage</p><h2>Vertragsorganisation</h2><p>Diese Angaben erscheinen in jedem neu freigegebenen Vertrag.</p></div><div className="contract-settings__fields"><label className="wide"><span>Rechtlicher Name</span><input required value={settings.legalName ?? ""} onChange={(event) => setSettings({ ...settings, legalName: event.target.value })}/></label><label className="wide"><span>Strasse</span><input required value={settings.street ?? ""} onChange={(event) => setSettings({ ...settings, street: event.target.value })}/></label><label><span>PLZ</span><input required value={settings.postalCode ?? ""} onChange={(event) => setSettings({ ...settings, postalCode: event.target.value })}/></label><label><span>Ort</span><input required value={settings.city ?? ""} onChange={(event) => setSettings({ ...settings, city: event.target.value })}/></label><label><span>Land</span><input required value={settings.country} onChange={(event) => setSettings({ ...settings, country: event.target.value })}/></label><label><span>Vertrags-E-Mail</span><input required type="email" value={settings.contactEmail ?? ""} onChange={(event) => setSettings({ ...settings, contactEmail: event.target.value })}/></label><label><span>Vertretungsberechtigte Person</span><input required value={settings.representativeName ?? ""} onChange={(event) => setSettings({ ...settings, representativeName: event.target.value })}/></label><label><span>Funktion</span><input required value={settings.representativeTitle ?? ""} onChange={(event) => setSettings({ ...settings, representativeTitle: event.target.value })}/></label><label><span>Verlängerung</span><select value={settings.renewalMode} onChange={(event) => setSettings({ ...settings, renewalMode: event.target.value as Settings["renewalMode"], noticeMonths: event.target.value === "annual_auto" ? 3 : null })}><option value="manual">Keine automatische Verlängerung</option><option value="annual_auto">Jährlich automatisch</option></select></label>{settings.renewalMode === "annual_auto" && <label><span>Kündigungsfrist in Monaten</span><input required min={1} max={12} type="number" value={settings.noticeMonths ?? 3} onChange={(event) => setSettings({ ...settings, noticeMonths: Number(event.target.value) })}/></label>}<label className="wide"><span>Gerichtsstand (optional)</span><input value={settings.placeOfJurisdiction ?? ""} onChange={(event) => setSettings({ ...settings, placeOfJurisdiction: event.target.value })}/></label></div><button className="access-primary" disabled={busy === "settings"}>{busy === "settings" ? "Speichert …" : "Vertragsangaben speichern"}</button></form>}
    {!settings.complete && !showSettings && <div className="contract-warning"><strong>Vertragsorganisation noch unvollständig</strong><p>Entwürfe sind möglich. Vor der Freigabe müssen Rechtsträger, Adresse, Vertretung und Kontakt vollständig sein.</p>{canManage && <button onClick={() => setShowSettings(true)}>Jetzt vervollständigen</button>}</div>}
