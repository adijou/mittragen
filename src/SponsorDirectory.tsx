...
import { matchesSearch, sortRows } from "./tableData";
...
export function SponsorDirectory({ tenantId, canWrite, onChanged }: {
  ...
  const sponsorSort = useTableSort<"logo" | "name" | "contact" | "status">("name");
  const overviewSort = useTableSort<string>("name");
  ...
  const allPackages = useMemo(() => packageOverview(sponsors).columns, [sponsors]);
  const filteredSponsors = sponsors.filter(sponsor =>
    (logoFilter === "all" || (logoFilter === "missing" ? !sponsor.logo_available : sponsor.logo_available))
    && (!statusFilter || sponsor.status === statusFilter)
    && (!packageFilter || (packageFilter === "none" ? !sponsor.package_assignments?.length : sponsor.package_assignments?.some(assignment => assignment.package_id === packageFilter)))
    && matchesSearch(search, sponsor.legal_name, sponsor.contact_name, sponsor.contact_email, sponsor.phone, sponsor.city, sponsor.source_organization, ...(sponsor.package_assignments ?? []).map(assignment => assignment.package_name)));
  const visibleSponsors = sortRows(filteredSponsors, sponsorSort.sort, (sponsor, key) => {
    switch (key) {
      case "logo": return sponsor.logo_available;
      case "name": return sponsor.legal_name;
      case "contact": return sponsor.contact_name || sponsor.contact_email || sponsor.phone;
      case "status": return statusLabels[sponsor.status] ?? sponsor.status;
    }
  });
  ...
  const overview = packageOverview(filteredSponsors);
  // Keep package columns stable while filters or sorting change the displayed sponsors.
  overview.columns = allPackages;
  const overviewRows = sortRows(overview.rows, overviewSort.sort, (row, key) => key === "name" ? row.sponsor.legal_name : key === "total" ? row.totalCents : row.amounts.get(key));
  ...
  return <section className="sponsor-directory">
    ...
    <div className="sponsor-toolbar">
      ...
      <label><span>Paket</span><select value={packageFilter} onChange={event => setPackageFilter(event.target.value)}><option value="">Alle Pakete</option><option value="none">Ohne bestätigtes Paket</option>{allPackages.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>
      <label><span>Status</span><select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="">Alle Status</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      <button type="button" className="table-reset" onClick={() => { setSearch(""); setLogoFilter("all"); setPackageFilter(""); setStatusFilter(""); sponsorSort.resetSort(); overviewSort.resetSort(); }}>Zurücksetzen</button>
    ...
    {showPackages && !loading && !error && <section className="sponsor-package-overview"><h2>Sponsoren und ihre Pakete</h2><p>Bestätigte Verträge einschliesslich Altbestand. Beträge in CHF pro Jahr; 0 bedeutet kostenlos, ein leeres Feld bedeutet kein zugeordnetes Paket. Entwürfe und aufgehobene Verträge sind nicht enthalten.</p><div className="sponsor-table-wrap"><table className="sponsor-table"><thead><tr><SortableHeader label="Sponsor" sortKey="name" sort={overviewSort.sort} onSort={overviewSort.toggleSort}/>{overview.columns.map(column => <SortableHeader key={column.id} label={column.name} sortKey={column.id} sort={overviewSort.sort} onSort={overviewSort.toggleSort}/>)}<SortableHeader label="Total CHF/Jahr" sortKey="total" sort={overviewSort.sort} onSort={overviewSort.toggleSort}/></tr></thead><tbody>{overviewRows.map(({ sponsor, amounts, totalCents }) => <tr key={sponsor.id}><td><strong>{sponsor.legal_name}</strong></td>{overview.columns.map(column => <td key={column.id}>{amounts.has(column.id) ? new Intl.NumberFormat("de-CH", { minimumFractionDigits: 2 }).format(amounts.get(column.id)! / 100) : "–"}</td>)}<td><strong>{new Intl.NumberFormat("de-CH", { minimumFractionDigits: 2 }).format(totalCents / 100)}</strong></td></tr>)}{overviewRows.length === 0 && <tr><td colSpan={overview.columns.length + 2}>Keine Sponsoren für die gewählten Filter.</td></tr>}</tbody></table></div><p>Der Export enthält immer alle Sponsoren der gewählten Organisation, auch ohne Paket.</p></section>}
    {loading ? <div className="sponsor-directory__state">Sponsoren werden geladen …</div> : error && !formOpen ? <div className="workspace-error"><strong>Die Sponsoren konnten nicht geladen werden.</strong><p>{error}</p></div> : visibleSponsors.length === 0 ? <div className="sponsor-directory__state"><strong>Noch keine passenden Sponsoren.</strong><p>{sponsors.length === 0 && canWrite ? "Erfassen Sie den ersten Sponsor für diese Organisation." : "Ändern Sie die Suche oder die Filter."}</p></div> : <div className="sponsor-table-wrap"><table className="sponsor-table"><thead><tr>{([['logo', 'Logo'], ['name', 'Sponsor'], ['contact', 'Kontakt'], ['status', 'Status']] as const).map(([key, label]) => <SortableHeader key={key} label={label} sortKey={key} sort={sponsorSort.sort} onSort={sponsorSort.toggleSort}/>)}<th scope="col"><span className="visually-hidden">Aktionen</span></th></tr></thead><tbody>{visibleSponsors.map((sponsor) => <tr key={sponsor.id}><td className="sponsor-logo-cell">{canWrite ? <button type="button" className="sponsor-logo-button" aria-label={`Logo von ${sponsor.legal_name} verwalten`} onClick={() => setLogoSponsor(sponsor)}><SponsorLogoPreview src={`/api/sponsors/${tenantId}/${sponsor.id}/logo?v=${encodeURIComponent(sponsor.logo_updated_at ?? "current")}`} name={sponsor.legal_name} available={sponsor.logo_available}/></button> : <SponsorLogoPreview src={`/api/sponsors/${tenantId}/${sponsor.id}/logo?v=${encodeURIComponent(sponsor.logo_updated_at ?? "current")}`} name={sponsor.legal_name} available={sponsor.logo_available}/>}</td><td><strong>{sponsor.legal_name}</strong><small>{[sponsor.postal_code, sponsor.city].filter(Boolean).join(" ") || sponsor.source_organization || "–"}</small></td><td><strong>{sponsor.contact_name || "–"}</strong><small>{sponsor.contact_email || sponsor.phone || "Kein Kontakt hinterlegt"}</small></td><td><span className={`sponsor-status sponsor-status--${sponsor.status}`}>{statusLabels[sponsor.status] ?? sponsor.status}</span></td><td>{canWrite && <div className="sponsor-row-actions"><button className="sponsor-edit" onClick={() => openEdit(sponsor)}>Bearbeiten</button><button className="sponsor-edit" onClick={() => setAccessSponsor(sponsor)}>Zugang einrichten</button></div>}</td></tr>)}</tbody></table></div>}
