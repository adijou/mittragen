...
function WorkspacePage({ user, setUser, onHome, onLogin, onPrototype }: { user: User | null; setUser: (user: User | null) => void; onHome: () => void; onLogin: () => void; onPrototype: () => void }) {
  ...
  const [workspaceSection, setWorkspaceSection] = useState<"overview" | "sponsors">("overview");

  useEffect(() => {
    if (!user) { setLoading(false); return; }
    api<{ tenants: Tenant[] }>("/api/tenants").then((result) => {
  ...
  const createTenant = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Organisation konnte nicht erstellt werden.");
    } finally {
      setCreating(false);
    }
  };
