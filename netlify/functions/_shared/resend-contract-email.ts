...
function emailShell(title: string, body: string) {
  return `<!doctype html><html lang="de"><body style="margin:0;background:#f4f5f1;color:#173127;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 16px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fff;border:1px solid #dfe5df;border-radius:18px;overflow:hidden"><tr><td style="padding:26px 32px;background:#173127;color:#fff"><div style="font-size:13px;font-weight:700;letter-spacing:.12em;color:#cbe8d8">MITTRAGEN</div><div style="margin-top:8px;font-size:24px;font-weight:700">${escapeHtml(title)}</div></td></tr><tr><td style="padding:32px">${body}</td></tr></table></td></tr></table></body></html>`;
}
...
export function buildContractSigningEmail(input: ContractSigningEmail) {
  ...
  const modeText = input.deliveryMode === "account"
    ? "Da diese E-Mail-Adresse bereits mit einem Mittragen-Konto verbunden ist, melden Sie sich bitte an. Ihre Identität wird über dieses Konto nachgewiesen."
  ...
  const html = emailShell("Vertrag zur Bestätigung", `<h1 style="margin:0 0 18px;font-size:25px">Guten Tag ${escapeHtml(input.signerName)}</h1><p style="font-size:16px;line-height:1.65">${escapeHtml(input.organizationName)} hat den Sponsoringvertrag <strong>${escapeHtml(input.contractNumber)}</strong> zur Bestätigung freigegeben.</p><p style="padding:18px;background:#f4f7f4;border-radius:12px;font-size:15px">${facts}</p><p style="font-size:16px;line-height:1.65">${modeText}</p><table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="border-radius:10px;background:#1f7a50"><a href="${escapeHtml(input.confirmationUrl)}" style="display:inline-block;padding:14px 22px;color:#fff;text-decoration:none;font-weight:700">${button}</a></td></tr></table><p style="margin:24px 0 0;font-size:13px;color:#53655d">Öffnen Sie zuerst das PDF und bestätigen Sie danach ausdrücklich den unveränderten Vertragsstand.</p>`);
  const text = `${subject}\n\nGuten Tag ${input.signerName}\n\n${input.organizationName} hat den Sponsoringvertrag ${input.contractNumber} zur Bestätigung freigegeben.\n${input.sponsorName} · ${input.packageName} · ${formatChf(input.annualValueCents)} pro Jahr\n\n${input.deliveryMode === "account" ? "Melden Sie sich mit Ihrem bestehenden Mittragen-Konto an. Ihre Identität wird über dieses Konto nachgewiesen." : `Ihr persönlicher Einmallink ist bis ${expiry} gültig.`}\n\n${input.confirmationUrl}`;
...
export function buildContractAccessEmail(input: ContractAccessEmail) {
  ...
  const html = emailShell("Ihr Sponsorzugang", `<h1 style="margin:0 0 18px;font-size:25px">Guten Tag ${escapeHtml(input.signerName)}</h1><p style="font-size:16px;line-height:1.65">Ihr bestehendes Mittragen-Konto wurde mit dem Sponsorbereich von <strong>${escapeHtml(input.sponsorName)}</strong> bei ${escapeHtml(input.organizationName)} verbunden.</p><table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="border-radius:10px;background:#1f7a50"><a href="${escapeHtml(input.portalUrl)}" style="display:inline-block;padding:14px 22px;color:#fff;text-decoration:none;font-weight:700">Sponsorbereich öffnen</a></td></tr></table>`);
  const text = `${subject}\n\nGuten Tag ${input.signerName}\n\nIhr bestehendes Mittragen-Konto wurde mit dem Sponsorbereich von ${input.sponsorName} bei ${input.organizationName} verbunden.\n\n${input.portalUrl}`;
  return { subject, html, text };
}
