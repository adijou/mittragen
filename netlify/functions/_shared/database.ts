...
export async function withSession<Row>(userId: string, tenantId: string | null, operation: (client: DatabaseClient) => Promise<Row>, userEmail?: string): Promise<Row> {
  ...
  try {
    ...
    if (userEmail) await client.query("SELECT set_config('app.user_email', $1, true)", [userEmail.trim().toLowerCase()]);
