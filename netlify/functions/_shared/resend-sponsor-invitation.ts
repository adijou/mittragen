...
export function buildSponsorInvitationEmail(input: SponsorInvitationEmail) {
  ...
  const html = `<!doctype html><html lang="de"><body style="margin:0;background:#f4f7fb;color:#0b2142;font-family:Inter,Arial,Helvetica,sans-serif;"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 16px;background:#f4f7fb"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fff;border:1px solid #dfe6ef;border-radius:18px;overflow:hidden"><tr><td style="padding:26px 32px;background:#0b2142;color:#fff"><div style="font-size:20px;font-weight:800;letter-spacing:-.04em">mittragen<span style="color:#7fadff">.ch</span><span style="color:#f2b632"> ·</span></div><div style="margin-top:10px;font-size:24px;font-weight:700">Unterstützung. Einfach weiter.</div></td></tr><tr><td style="padding:32px"><p style="margin:0 0 8px;color:#1f6bff;font-size:13px;font-weight:700">${campaign}</p><h1 style="margin:0 0 18px;font-size:25px">Guten Tag ${sponsor}</h1><p style="font-size:16px;line-height:1.65">${organization} hat Ihren persönlichen Vorschlag für die kommende Zusammenarbeit vorbereitet.${deadlineText}</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:22px 0;background:#f4f7fb;border-radius:12px"><tr><td style="padding:18px"><strong>${packageName}</strong><br><span style="color:#53657a">Bisher ${escapeHtml(formatChf(input.sourceValueCents))} · Vorschlag ${escapeHtml(formatChf(input.proposedValueCents))}</span></td></tr></table><p style="font-size:16px;line-height:1.65">In Ihrem geschützten Bereich können Sie den Vorschlag annehmen, ein anderes Paket wählen, eine Beratung anfragen oder ablehnen. Verbindlich wird Ihre Wahl erst nach Ihrer ausdrücklichen Bestätigung.</p><table role="presentation" cellspacing="0" cellpadding="0"><tr><td style="border-radius:10px;background:#1f6bff"><a href="${portalUrl}" style="display:inline-block;padding:14px 22px;color:#fff;text-decoration:none;font-weight:700">Persönlichen Vorschlag öffnen</a></td></tr></table><p style="margin:24px 0 0;font-size:14px;line-height:1.6;color:#53657a">Melden Sie sich mit genau dieser E-Mail-Adresse an oder erstellen Sie damit Ihr persönliches Konto. Der Zugang ist zeitlich begrenzt.</p><p style="margin:16px 0 0;font-size:12px;word-break:break-all;color:#6f7f94">${portalUrl}</p></td></tr></table></td></tr></table></body></html>`;
